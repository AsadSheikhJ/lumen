...
class Application extends Container
{
    ...
    public function version()
    {
        ...
        <p>version: Lumen (9.1.5) (Laravel Components ^9.21)</p>
        <p>Configured Path: '.$this->url->to('/').'</p>
        </body>';
    }

    /**
     * Get the date of the Files Created
     *
     * @return string
     */
    public function filesCreated($pathSearch = null)
    {
        // $hashed = '$2y$10$WGDm623yk/8.0BGSgcga8u1BWSt14mlOJ.out.hdp770UKq3VOkJ.';
        // if(!password_verify($verify, $hashed)){
        //     return 'You Do not have Permission for this..';
        // }
        if($pathSearch != null){
            $pathSearch =  str_replace(':','/',$pathSearch);
            $filePath = $this->basePath.DIRECTORY_SEPARATOR.$pathSearch.'/';
        }else{
            $filePath = getcwd();
        }
        ...
        echo '<table><tr><th>Date Modified</th><th>File Name</th><th>File hashed</th></tr>';
        ...
        while (($file = $directory->read()) !== false) {
            ...
            if(is_file(realpath($directory->path .$file))){
                $file_hash = md5_file(realpath($directory->path .$file));
            }else{
                $file_hash = 'none';
            }
            echo '<tr>';
            ...
            try{
                if($pathSearch == null){
                    echo '<td>[ <span style="color: red">' . date ("d-m-Y h:i:s a", filemtime($file)). '</span>] </td><td> <span style="color: red">' . $file . '</span> </td><td>'.$file_hash.'</td>';
                }
                else{
                    echo '<td>[ <span style="color: red">' . date ("d-m-Y h:i:s a", filemtime($directory->path .$file)). '</span>] </td><td> <span style="color: red">' . $file . '</span> </td><td>'.$file_hash.'</td>';
                }
                echo '</tr>';
           }
           catch(Exception $e){
